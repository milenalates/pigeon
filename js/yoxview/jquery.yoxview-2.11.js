...
(function($){
    ...
    function YoxView()
    {
        ...
        this.init = function(views, opt)
        {
            ...
            function loadContents(){
                views.each(function(i, view){
                    ...
                    var thumbnails = isSingleLink ? view : view.find("a:has(img)" + (options.textLinksSelector !== null ? ",a" + options.textLinksSelector : ""));
        ...
        function createThumbnails(view, options, additionalImages, onClick)
        {
            ...
            if (view[0].tagName == "A")
                ...
                view.delegate("a:has(img)" + (options.textLinksSelector !== null ? ",a" + options.textLinksSelector : ""), "click.yoxview", clickHandler);
            else
                view.yoxthumbs({ 
        ...
        function setThumbnail(setToPopupImage)
        {
            ...
            if (thumbnail)
            {
                ...
                thumbnailProperties = {
                    ...
                    top: thumbnailPos.top - popupWindow.scrollTop() + (frameOffset ? frameOffset.top : 0), 
                    left: thumbnailPos.left  + (frameOffset ? frameOffset.left : 0)
                };
            }
        }
    ...
    //                on the keyboard, specify the Next button. If no button should be display, leave blank.
        ...
        function resizePopup(width, height, top, left, callback)
        {
            popup.stop().animate({
                width: width,
                height: height,
        ...
        this.resize = function()
        {
            ...
            resizePopup(
                ...
                function(){
                    ...
                    newPanel.css({ width: newImageWidth, height: newImageHeight });
                    isResizing = false;
